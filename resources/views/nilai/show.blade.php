                    ...
                    <table id="myTable" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            {{-- @foreach ($kategori as $key => $item)
                                <!-- <li class="list-group-item"><b>{{ $item->name }} : </b>{{ $item->nilai }}</li> -->
                                <tr>
                                    <td>{{ ($kategori->currentPage() - 1) * $kategori->perPage() + $key + 1 }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>
                                        <input class="form-control form-control-sm" name="nilai"
                                            value="{{ $item->nilai ?? 0 }}" type="number" />
                                    </td>
                                    <td>
                                        <input class="form-control form-control-sm" name="keterangan"
                                            value="{{ $item->keterangan }}" type="text" />
                                    </td>
                                    <td>
                                        @can('nilai.edit')
                                            @if ($item->id)
                                                <form action="{{ route('nilai.delNilai', $item->id) }}" method="POST">
                                                    ...
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="btn btn-danger btn-sm btn-icon confirm-delete"><i
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return `<input class="form-control form-control-sm" name="nilai"
                                            value="` + (data ?? 0) + `" type="number" />`
                        }
                    },
                    {
                        data: 'keterangan',
                        name: 'keterangan',
                        render: function(data, type, row, meta) {
                            return `<input class="form-control form-control-sm" name="keterangan"
                    ...
                    },
                    {
                        ...
                        searchable: true,
                        render: function(data, type, row, meta) {
                            return row.id != undefined ? `<button type='submit'
                                                        class='btn btn-danger btn-sm btn-icon confirm-delete'><i
                                                            class='fas fa-trash'></i></button>` : "";
                        },
                    },
            ...
            $('#myModal').on('show.bs.modal', function(e) {
                ...
                fetchData(id).done(function(data) {
                    ...
                    $.each(data, function(index, item) {
                        select2Element.append($('<option></option>').attr('value', item.id)
                            .text(item.name));
                    });
