        ...
        <div class="card">
            <form action="{{ isset($siswa) ? route('siswa.update', $siswa) : route('siswa.store') }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <x-form.input type="number" name="user_id" :value="isset($user) ? $user->user_id : ''" label="ID User" />
                    <x-form.select-custom name="kelas_id" :value="isset($siswa) ? $siswa->kelas_id : ''" label="ID Kelas" >
                        @foreach ($kelas as $item)
                        <option value="{{ $item->id }}" {{ isset($siswa) ? $siswa->kelas_id === $item->id ? 'selected' : '':'' }}>{{ $item->nama }}</option>
                        @endforeach
                    </x-form.select-custom>
                    <x-form.input name="smk_id" :value="isset($user) ? $user->smk_id : ''" label="ID SMK" />
                    <x-form.input name="nama" :value="isset($user) ? $user->nama : ''" label="Nama" />
