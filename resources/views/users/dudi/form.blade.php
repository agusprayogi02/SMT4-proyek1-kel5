        ...
        <div class="card">
            ...
            <form action="{{ isset($dudi) ? route('dudi.update', $dudi->nib) : route('dudi.store') }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <x-form.input name="nib" :value="isset($dudi) ? $dudi->nib : old('nib')" label="NIB"
                        :readonly="isset($dudi)" />
                    <x-form.input name="nama" :value="isset($dudi) ? $dudi->nama : old('nama')" label="Nama" />
                    <x-form.input name="email" :value="isset($dudi) ? $dudi->user->email : old('email')"
                        label="Email" />
                    @if(!isset($dudi))
                    <x-form.input name="password" :value="old('password')" label="Password" type="password" />
                    @endif
                    <x-form.input name="nama_pemilik" :value="isset($dudi) ? $dudi->nama_pemilik : old('nama_pemilik')"
                        label="Nama Pemilik" />
                    <x-form.text-area name="alamat" :value="isset($dudi) ? $dudi->alamat : old('alamat')" label="Alamat"
                        placeholder="Masukkan Alamat" />
                    <x-form.input type="number" name="no_telp" :value="isset($dudi) ? $dudi->no_telp : old('no_telp')"
                        label="No Telpon DuDi" />
                    <x-form.input type="number" name="kuota" :value="isset($dudi) ? $dudi->kuota : old('kuota')"
                        label="Jumlah Kuota Magang" />
                </div>
