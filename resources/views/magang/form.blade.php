        ...
        <div class="card">
            <form action="{{ isset($magang) ? route('magang.update', $magang) : route('magang.store') }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md">
                            <x-form.select2 name="dudi_id" :value="isset($magang) ? $magang->dudi_id : ''" label="DUDI">
                                ...
                                <option value="{{ $item->nib}}" {{ (isset($magang) && $item->nib == $magang->dudi_id
                                    ||
                                    old('dudi_id') == $item->nib)?
                                    'selected': '' }}>{{ $item->nama }}</option>
                                @endforeach
                            </x-form.select2>
                        </div>
                        <div class="col-md">
                            <x-form.select2 name="guru_id" :value="isset($magang) ? $magang->guru_id : ''"
                                label="Guru Pembimbing">
                                @foreach ($guru as $item)
                                <option value="{{ $item->nip}}" {{ (isset($magang) && $item->nip == $magang->guru_id
                                    ||
                                    old('guru_id') == $item->nip)?
                                    'selected': '' }}>{{ $item->nama }}</option>
                                @endforeach
                            </x-form.select2>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md">
                            <x-form.select2 name="keahlian_id" :value="isset($magang) ? $magang->keahlian_id : ''"
                                label="Bidang Keahlian">
                                @foreach ($keahlian as $item)
                                <option value="{{ $item->id}}" {{ (isset($magang) && $item->id == $magang->keahlian_id
                                    ||
                                    old('keahlian_id') == $item->id)?
                                    'selected': '' }} >{{ $item->nama }}</option>
                                @endforeach
                            </x-form.select2>
                        </div>
                        <div class="col-md">
                            <x-form.text-area name="alasan" :value="isset($magang) ? $magang->alasan : old('alasan')"
                                label="Alasan"
